...
get_params() {
    ...
    local LR_IDX=$((BATCH_IDX / 20))
    local SEED_IDX=$(((BATCH_IDX / 4) % 5))
    local SUB_BATCH_IDX=$((BATCH_IDX % 4))
    
    local EST_IDX=$CONFIG_IN_BATCH
    local NXK_IDX=$SUB_BATCH_IDX
